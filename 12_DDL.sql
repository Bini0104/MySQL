...
-- 열 추가
ALTER TABLE tb2
ADD col2 INT NOT NULL; -- ADD 추가

DESCRIBE tb2;

-- 열 이름 및 데이터 형식 변경
ALTER TABLE tb2
CHANGE COLUMN fk change_fk INT NOT NULL; -- CHANGE 변경

-- 열 제약 조건 추가 및 삭제
ALTER TABLE tb2
DROP PRIMARY KEY; 	-- 에러 발생  DROP 삭제

-- AUTO_INCREMENT가 걸려 있는 컬럼은 PRIMARY KEY 제거가 안되므로
-- AUTO_INCREMENT를 MODIFY(수정하다) 명령어로 제거한다.
...
-- DROP 이후 바로 테이블을 재생성 해주는 TRUNCATE가 훨씬 효율적으로 한번에 테이블을 초기화 시켜준다. 구조를 남기고 내용만 비운다.
-- drop은 테이블을 삭제하기에
